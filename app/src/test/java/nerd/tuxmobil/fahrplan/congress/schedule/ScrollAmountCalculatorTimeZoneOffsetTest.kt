...
/**
 * Covers ScrollAmountCalculator.calculateScrollAmount(Session, Int).
...
class ScrollAmountCalculatorTimeZoneOffsetTest(
    ...
    fun calculateScrollAmount() {
        withTimeZone(deviceTimeZoneId) {
            ...
            val scrollAmount = createCalculator(sessions).calculateScrollAmount(targetSession, BOX_HEIGHT)
            assertThat(scrollAmount).isEqualTo(expectedScrollAmount)
        }
    }

    private fun createCalculator(sessions: List<Session>): ScrollAmountCalculator {
